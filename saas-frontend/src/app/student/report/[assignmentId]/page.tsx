...
import { TrendingUp, Target, BrainCircuit, ShieldAlert, CheckCircle2, ChevronLeft, XCircle, MinusCircle, ListOrdered } from "lucide-react";
...
export default function StudentReportView() {
  ...
  if (loading) return <div className="p-12 text-center text-surface-500 font-medium">Loading your detailed analysis...</div>;
  if (!assignment) return <div className="p-12 text-center text-red-500 font-medium">Report not found.</div>;
  ...
  const skillsData = reportData.skills.map((s: string, i: number) => ({
    name: s,
  ...
  const maxPossibleMarks = assignment.paper?.maxMarks || 100;

  return (
    <div className="p-4 md:p-8 space-y-6 max-w-7xl mx-auto pb-12">
      <Link href="/student/dashboard" className="inline-flex items-center gap-2 text-surface-500 hover:text-surface-900 transition mb-2">
      ...
      <div className="bg-gradient-to-br from-brand-900 via-brand-800 to-brand-600 rounded-3xl p-8 md:p-12 text-white shadow-2xl relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="relative z-10 space-y-2 text-center md:text-left">
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-2">Detailed Analysis</h1>
          <p className="text-brand-100 text-lg">{assignment.paper?.title}</p>
          <div className="inline-block mt-4 px-4 py-2 bg-white/10 backdrop-blur-md rounded-full border border-white/20">
            <span className="text-brand-50 font-medium">Completed on {new Date(assignment.completedAt).toLocaleDateString()}</span>
          </div>
        </div>
          ...
          <div className="bg-orange-50 rounded-2xl shadow-sm border border-orange-100 p-6">
            ...
            <ul className="space-y-3">
              ...
              {reportData.unattemptedCount > 0 && (
                <li className="flex items-start gap-3 text-orange-700 font-medium">
                  <span className="w-1.5 h-1.5 rounded-full bg-orange-500 mt-2 shrink-0" />
                  {reportData.unattemptedCount > 2 ? "Time management needs work—try not to leave questions blank." : "Double-check your answers before time expires. Unattempted questions missed out on potential marks."}
                </li>
              )}
            </ul>
          </div>
              ...
              <div key={idx} className={`p-6 rounded-2xl border ${
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div className={`p-3 rounded-xl border ${q.status === 'correct' ? 'bg-emerald-100/50 border-emerald-200 text-emerald-900' : q.status === 'incorrect' ? 'bg-rose-100/50 border-rose-200 text-rose-900' : 'bg-slate-100 border-slate-200 text-slate-600'}`}>
                    <span className="block text-xs uppercase font-bold opacity-70 mb-1">Your Answer</span>
